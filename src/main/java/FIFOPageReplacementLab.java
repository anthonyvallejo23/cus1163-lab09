...
public class FIFOPageReplacementLab {
    ...
    public static int[] simulateFIFO(int[] referenceString, int numFrames) {
        ...
        Queue<Integer> memoryFrames = new LinkedList<>(); // Represents frames in memory
        Set<Integer> pagesInMemory = new HashSet<>(); // Represents page in memory

        for (int page : referenceString) {
            if (pagesInMemory.contains(page)) {
                pageHits++; // If page found in memory, increment the hits.
            } else {
                pageFaults++; // If page not in memory, increment faults.
                if (memoryFrames.size() == numFrames) {
                    // If the frames are full,
                    int victim = memoryFrames.poll(); // Remove the oldest page and remove the victim from the set.
                    pagesInMemory.remove(victim);
                }
                memoryFrames.offer(page); // Add the new page to both the queue and the set.
